...
- (void)testEncode {
  ...
  ZXBitMatrix *result = [[[ZXUPCAWriter alloc] init] encode:@"485963095124" format:kBarcodeFormatUPCA
                                                                    width:testStr.length height:0 error:nil];
...
- (void)testAddChecksumAndEncode {
  ...
  ZXBitMatrix *result = [[[ZXUPCAWriter alloc] init] encode:@"12345678901" format:kBarcodeFormatUPCA
                                                                    width:testStr.length height:0 error:nil];
  for (int i = 0; i < testStr.length; i++) {
    BOOL expected = [testStr characterAtIndex:i] == '1';
    STAssertEquals([result getX:i y:0], expected, @"Expected (%d, 0) to be %d", i, expected);
